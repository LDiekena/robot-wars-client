...
import io.swagger.client.View.ClientView;
...
public class ClientMain {
    ...
    public static void main(String[] args) throws ApiException, InterruptedException {
        ...
        //TODO: Alternativ einem bereits gestarteten Spiel wieder beitreten, Abfrage nach gameID und playerID
        ...
        while (!isTurnOver(currentMovementRate, getLastMoveFromPlayer(playerID, gameID, defaultApi), gameID)) {
            ...
            if(turnChoice == 1) {
                ...
                //TODO: in Switch ändern, Mapgröße bisher nicht variabel!
                if(directionChoice.equals("w")  && lastMove.getMapIndex().intValue() >= 8 && lastMove.getAlign().equals(Align.N)) {
                    newMove.setMapIndex(BigDecimal.valueOf(lastMove.getMapIndex().intValue() - 9));
                } else if (directionChoice.equals("a") && (lastMove.getMapIndex().intValue() % 9) == 0 && lastMove.getAlign().equals(Align.W)) {
                    newMove.setMapIndex(BigDecimal.valueOf(lastMove.getMapIndex().intValue() - 1));
                } else if (directionChoice.equals("s") && lastMove.getMapIndex().intValue() < 36 && lastMove.getAlign().equals(Align.S)) {
                    newMove.setMapIndex(BigDecimal.valueOf(lastMove.getMapIndex().intValue() + 9));
        ...
        //Abfrage Roboter erstellen oder auswählen (Liste)
        //INPUT Userauswahl
        //Get robots --> Auswahl Zwischenvariable im Client für Spiel
        //POST createRobot


        //Abfrage Spiel erstellen oder beitreten
        //INPUT Userauswahl
        //POST createGame mit Zwischenvariable RobotID
        //POST joinGame mit Zwischenvariable RobotID

        //GET game bis Spiel STARTED

        //GET moves/ GET moves after
